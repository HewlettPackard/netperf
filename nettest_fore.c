...
#endif /* lint */
#ifdef DO_FORE
char	nettest_fore[]="@(#)nettest_fore.c (c) Copyright 1994, \
...
%5d            %-7.2f   %7d           %7.1f      %-6.2f  %-6.3f\n\n";
  ...
  if (sigaction(SIGALRM, &action, NULL) < 0) {
    fprintf(where,"send_fore_stream: error creating alarm signal.\n");
    fflush(where);
    exit(1);
  }
#else /* SUNOS4 */
...
recv_fore_stream()
{
  ...
  action.sa_flags = SA_INTERRUPT;
  if (sigaction(SIGALRM, &action, NULL) < 0) {
    fprintf(where,"recv_fore_stream: error creating alarm signal.\n");
    fflush(where);
    exit(1);
  }
#else /* SUNOS4 */
...
#define	MAX_KEPT_TIMES	1024
  ...
  if (test_time) {
    ...
    action.sa_flags = SA_INTERRUPT;
    if (sigaction(SIGALRM, &action, NULL) < 0) {
      fprintf(where,"send_fore_rr: error creating alarm signal.\n");
      fflush(where);
      exit(1);
    }
#else /* SUNOS4 */
...
recv_fore_rr()
{
  ...
  if (fore_rr_request->test_length > 0) {
    ...
    action.sa_flags = SA_INTERRUPT; 
    if (sigaction(SIGALRM, &action, NULL) < 0) {
      fprintf(where,"recv_fore_rr: error creating alarm signal.\n");
      fflush(where);
      exit(1);
    }
#else /* SUNOS4 */
    signal(SIGALRM, catcher);
#endif /* SUNOS4 */
    alarm(fore_rr_request->test_length + PAD_TIME);
  }
  else {
    times_up = 1;
